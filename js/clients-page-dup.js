...
async function loadClients() {
  ...
  try {
    ...
    byId("loading-state").textContent =
      `Failed to load clients: ${formatErr(e)}`;
  }
}
...
function renderClients(list) {
    ...
    tr.innerHTML = `
      <td>
        <div class="client-name">
          <a href="index.html?id=${c.id}">${escape(c.household_name)}</a>
        </div>
      </td>
  ...
  tbody
    .querySelectorAll('button[data-action="archive"]')
    .forEach((b) =>
...
function buildLabelOptions(kind) {
  ...
  return (
    ...
    labels
      .map(
        (l) =>
          `<option value="${escape(l)}">${escape(LABEL_TITLES[l] || l)}</option>`,
      )
      .join("")
  );
}
...
function addAccountRow(account = null) {
  ...
  row.innerHTML = `
    ...
    <div class="field">
      <input type="text" name="acct_rate_${i}" placeholder="e.g. 5.25" inputmode="decimal" />
    </div>
...
function applyOwnerConstraints(row) {
  ...
  if (!ownerEl) return;
  const spouseOpt = Array.from(ownerEl.options).find(
    (o) => o.value === "spouse",
  );
...
function collectAccounts() {
  ...
  $$(".account-row").forEach((row) => {
    ...
    const institution =
      row.querySelector(`[name="acct_institution_${i}"]`)?.value?.trim() ||
      null;
...
function buildPayload() {
  ...
  return {
    ...
    static_financials: {
      ...
      pr_target: byId("pr-target").value.trim()
        ? parseFloat(byId("pr-target").value)
        : null,
...
function validatePayload(p) {
  ...
  if (!p.client.first_name || !p.client.last_name)
    return "Client first + last name are required.";
  if (
    p.household_type === "married" &&
    (!p.spouse?.first_name || !p.spouse?.last_name)
  ) {
...
async function saveForm() {
  ...
  try {
    if (id) {
      ...
      closeDrawer();
      window.location.href = `index.html?id=${created.id}`;
      return;
    }
    closeDrawer();
    await loadClients();
...
async function archiveClient(id) {
  if (!confirm("Archive this client? They will be hidden from the list."))
    return;
  try {
